...
<div class="block_in_wrapper">
	...
	<div class="row">
		<label for="end">{$langfile.due}:</label><input type="text" class="text" value="{$project.endstring}" name="end"  id="end"  {if $project.end == 0}disabled = "disabled"{/if} realname="{$langfile.due}" />
		<br /><br />
		<label for="neverdue"></label><input type = "checkbox" class = "checkbox" value = "neverdue" name = "neverdue" id = "neverdue" {if $project.end == 0}checked = "checked" onclick = "$('end').enable();"{else}onclick = "$('end').value='';$('end').disabled='disabled';"{/if}><label for = "neverdue">{$langfile.neverdue}</label>
	</div>
